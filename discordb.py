...
class DiscordBackend(ErrBot):
    ...
    def send_message(self, msg):
        ...
        for message in [msg.body[i:i+DISCORD_MESSAGE_SIZE_LIMIT] for i in range(0, len(msg.body), DISCORD_MESSAGE_SIZE_LIMIT)]:
            discord.compat.run_coroutine_threadsafe(self.client.send_typing(recipient), loop=self.client.loop)
            discord.compat.run_coroutine_threadsafe(self.client.send_message(destination=recipient, content=message), loop=self.client.loop)
    ...
    def send_card(self, card):
        ...
        discord.compat.run_coroutine_threadsafe(self.client.send_typing(recipient), loop=self.client.loop)
        discord.compat.run_coroutine_threadsafe(self.client.send_message(destination=recipient, embed=em), loop=self.client.loop)
